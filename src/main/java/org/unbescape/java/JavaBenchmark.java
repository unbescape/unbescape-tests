 ...
 *
 */
public final class JavaBenchmark {
    ...
    static {
        ...
        for (int i = 0; i < ESCAPE_TEXTS.length; i++) {
            ...
            ESCAPE_TEXTS_UNBESCAPE[i] = JavaEscape.escapeJava(ESCAPE_TEXTS[i]);
            ESCAPE_NEW_OBJECT_UNBESCAPE[i] = (ESCAPE_TEXTS[i] == ESCAPE_TEXTS_UNBESCAPE[i]);

            ESCAPE_TEXTS_STRING_ESCAPE_UTILS[i] = StringEscapeUtils.escapeJava(ESCAPE_TEXTS[i]);
        ...
        for (int i = 0; i < UNESCAPE_TEXTS.length; i++) {
            ...
            UNESCAPE_TEXTS_UNBESCAPE[i] = JavaEscape.unescapeJava(UNESCAPE_TEXTS[i]);
            UNESCAPE_NEW_OBJECT_UNBESCAPE[i] = (UNESCAPE_TEXTS[i] == UNESCAPE_TEXTS_UNBESCAPE[i]);

            UNESCAPE_TEXTS_STRING_ESCAPE_UTILS[i] = StringEscapeUtils.unescapeJava(UNESCAPE_TEXTS[i]);
    ...
    private static void warmup(final Writer writer) throws IOException {
        ...
        for (int i = 0; i < warmupIters; i++) {
            final String res01 = JavaEscape.escapeJava(ESCAPE_TEXTS[i % ESCAPE_TEXTS.length]);
            final String res02 = StringEscapeUtils.escapeJava(ESCAPE_TEXTS[i % ESCAPE_TEXTS.length]);
        }
        for (int i = 0; i < warmupIters; i++) {
            final String res01 = JavaEscape.unescapeJava(UNESCAPE_TEXTS[i % UNESCAPE_TEXTS.length]);
            final String res02 = StringEscapeUtils.unescapeJava(UNESCAPE_TEXTS[i % UNESCAPE_TEXTS.length]);
        }
    ...
    private static void performEscapeBenchmark(final int n, final Writer writer) throws IOException {
        ...
        for (int i = 0; i < BENCHMARK_EXECS; i++) {
            final String result = JavaEscape.escapeJava(ESCAPE_TEXTS[n]);
        }
        final long ufinish = System.nanoTime();

        final long cstart = System.nanoTime();
        for (int i = 0; i < BENCHMARK_EXECS; i++) {
            final String result = StringEscapeUtils.escapeJava(ESCAPE_TEXTS[n]);
        }
    ...
    private static void performUnescapeBenchmark(final int n, final Writer writer) throws IOException {
        ...
        final long ustart = System.nanoTime();
        for (int i = 0; i < BENCHMARK_EXECS; i++) {
            final String result = JavaEscape.unescapeJava(UNESCAPE_TEXTS[n]);
        }
        final long ufinish = System.nanoTime();

        final long cstart = System.nanoTime();
        for (int i = 0; i < BENCHMARK_EXECS; i++) {
            final String result = StringEscapeUtils.unescapeJava(UNESCAPE_TEXTS[n]);
        }
        final long cfinish = System.nanoTime();
    ...
    public static void main(String[] args) throws Exception {
        ...
        writer.write("[BENCHMARK] STARTING UNBESCAPE BENCHMARK FOR: JAVA\n");
        writer.write("[BENCHMARK] --------------------------------------\n");
