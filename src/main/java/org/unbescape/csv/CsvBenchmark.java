...
public final class CsvBenchmark {
    ...
    private static final String[] UNESCAPE_TEXTS =
            new String[] {
                    "\"<\"\"[\u0163\\xE1aeiouABC0123\uD840\uDC00\']\uD835\uDCCD>\"",
                    "\"<\"\"[\u0163\\xE1&aeiouABC0123\uD840\uDC00\']\uD835\uDCCD>\"",
                    "\"<\"\"[aeiouABC0123\']>\"",
                    "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"
            };
    // Arrays containing textual results
    ...
    static {
        ...
        for (int i = 0; i < ESCAPE_TEXTS.length; i++) {
            ...
            ESCAPE_TEXTS_UNBESCAPE[i] = CsvEscape.escapeCsv(ESCAPE_TEXTS[i]);
            ESCAPE_NEW_OBJECT_UNBESCAPE[i] = (ESCAPE_TEXTS[i] == ESCAPE_TEXTS_UNBESCAPE[i]);

            ESCAPE_TEXTS_STRING_ESCAPE_UTILS[i] = StringEscapeUtils.escapeCsv(ESCAPE_TEXTS[i]);
        ...
        for (int i = 0; i < UNESCAPE_TEXTS.length; i++) {
            ...
            UNESCAPE_TEXTS_UNBESCAPE[i] = CsvEscape.unescapeCsv(UNESCAPE_TEXTS[i]);
            UNESCAPE_NEW_OBJECT_UNBESCAPE[i] = (UNESCAPE_TEXTS[i] == UNESCAPE_TEXTS_UNBESCAPE[i]);

            UNESCAPE_TEXTS_STRING_ESCAPE_UTILS[i] = StringEscapeUtils.unescapeCsv(UNESCAPE_TEXTS[i]);
    ...
    private static void warmup(final Writer writer) throws IOException {
        ...
        for (int i = 0; i < warmupIters; i++) {
            final String res01 = CsvEscape.escapeCsv(ESCAPE_TEXTS[i % ESCAPE_TEXTS.length]);
            final String res02 = StringEscapeUtils.escapeCsv(ESCAPE_TEXTS[i % ESCAPE_TEXTS.length]);
        }
        for (int i = 0; i < warmupIters; i++) {
            final String res01 = CsvEscape.unescapeCsv(UNESCAPE_TEXTS[i % UNESCAPE_TEXTS.length]);
            final String res02 = StringEscapeUtils.unescapeCsv(UNESCAPE_TEXTS[i % UNESCAPE_TEXTS.length]);
        }
    ...
    private static void performEscapeBenchmark(final int n, final Writer writer) throws IOException {
        ...
        for (int i = 0; i < BENCHMARK_EXECS; i++) {
            final String result = CsvEscape.escapeCsv(ESCAPE_TEXTS[n]);
        }
        final long ufinish = System.nanoTime();

        final long cstart = System.nanoTime();
        for (int i = 0; i < BENCHMARK_EXECS; i++) {
            final String result = StringEscapeUtils.escapeCsv(ESCAPE_TEXTS[n]);
        }
    ...
    private static void performUnescapeBenchmark(final int n, final Writer writer) throws IOException {
        ...
        final long ustart = System.nanoTime();
        for (int i = 0; i < BENCHMARK_EXECS; i++) {
            final String result = CsvEscape.unescapeCsv(UNESCAPE_TEXTS[n]);
        }
        final long ufinish = System.nanoTime();

        final long cstart = System.nanoTime();
        for (int i = 0; i < BENCHMARK_EXECS; i++) {
            final String result = StringEscapeUtils.unescapeCsv(UNESCAPE_TEXTS[n]);
        }
        final long cfinish = System.nanoTime();
